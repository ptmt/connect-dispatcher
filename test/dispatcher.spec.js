...
describe('Dispatcher', function() {
  ...
  describe('default configuration', function() {
    ...
    it('should response with 404 if controller does not exist', function(
      done) {
      requestApp()
    ...
    it(
      'should response with 404 if aciton in controlller without __missing_action does not exist',
      function(done) {
        requestApp()
          .post('/auth/index')
          .expect(404, done);
    ...
    it('should response with 404 if controller already cached', function(
      done) {
      requestApp()
        .get('/auth/index')
        .end(function() {
    ...
    it('should response with json when ?json=1 attached to uri', function(
      done) {
      requestApp()
    ...
    it(
      'should response with json when method is POST and view is not exist',
      function(done) {
        requestApp()
          .post('/pages/no_view')
          .end(function(err, res) {
    ...
    it(
      'should response with json when view is not exist and ?json=1 specified',
      function(done) {
        requestApp()
          ...
          .end(function(err, res) {
            ...
            var json = JSON.parse(res.text);
            expect(json).have.to.equal(
              'pages_controller#no_view_and_async');
            done(err);
          });
      });

    it('should response with 404 method call this.error404()', function(
      done) {
      requestApp()
        .get('/pages/return404')
        .expect(404, done);
  ...
  describe('__missing_controller', function() {
    ...
    it('should dispatch to specific controller according to options.routes',
      function(done) {
        ...
        .end(function(err, res) {
          ...
          requestAppMissingCountroller()
            ...
            .end(function(err1, res1) {
              ...
              expect(json.flash.message).have.to.equal(
                'some_kind_of_error');
              done(err);
            });
  ...
  describe('Stability', function() {
    ...
    it('ok with two callbacks', function(done) {
      var app = connect();
